...
class PHPMailer {
    ...
    protected function sendmailSend($header, $body) {
        ...
        if ($this->SingleTo) {
            foreach ($this->SingleToArray as $toAddr) {
                ...
                $result = pclose($mail);
                $this->doCallback(
                        ($result == 0), array($toAddr), $this->cc, $this->bcc, $this->Subject, $body, $this->From, array()
                );
                if (0 !== $result) {
                    throw new Exception($this->lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
                }
            ...
            $result = pclose($mail);
            $this->doCallback(
                    ($result == 0), $this->to, $this->cc, $this->bcc, $this->Subject, $body, $this->From, array()
            );
            if (0 !== $result) {
                throw new Exception($this->lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
            }
    ...
    protected function smtpSend($header, $body) {
        ...
        foreach ($callbacks as $cb) {
            $this->doCallback(
                    $cb['issent'], array($cb['to']), array(), array(), $this->Subject, $body, $this->From, array('smtp_transaction_id' => $smtp_transaction_id)
            );
        }
            ...
            try {
                ...
                if (empty($this->sign_extracerts_file)) {
                    $sign = @openssl_pkcs7_sign(
                                    $file, $signed, 'file://' . realpath($this->sign_cert_file), array('file://' . realpath($this->sign_key_file), $this->sign_key_pass), array()
                    );
                } else {
                    $sign = @openssl_pkcs7_sign(
                                    $file, $signed, 'file://' . realpath($this->sign_cert_file), array('file://' . realpath($this->sign_key_file), $this->sign_key_pass), array(), PKCS7_DETACHED, $this->sign_extracerts_file
                    );
                }
                fclose($file);
                if ($sign) {
